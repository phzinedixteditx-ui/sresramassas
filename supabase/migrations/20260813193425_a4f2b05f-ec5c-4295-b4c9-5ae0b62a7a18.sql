...
BEGIN
  ...
  IF lower(trim(coalesce(p_size,''))) = 'pequeno' THEN v_base := 24; v_limit := 6; v_size := 'Pequeno';
  ELSIF lower(trim(coalesce(p_size,''))) = 'grande' THEN v_base := 27; v_limit := 8; v_size := 'Grande';

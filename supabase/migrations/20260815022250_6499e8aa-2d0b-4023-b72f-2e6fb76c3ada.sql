...
CREATE OR REPLACE FUNCTION public.create_order(p_customer_name text, p_phone text, p_order_type text, p_address text, p_number text, p_complement text, p_neighborhood text, p_reference text, p_size text, p_pasta_type text, p_sauce text, p_ingredients text[], p_shrimp boolean, p_saute_type text, p_finishing text[], p_notes text)
...
BEGIN
  ...
  IF p_order_type <> 'local' AND length(regexp_replace(coalesce(p_phone,''), '\D', '', 'g')) < 10 THEN
    RAISE EXCEPTION 'Telefone inválido';
  END IF;

  v_size := lower(trim(coalesce(p_size,'')));
  IF v_size = 'pequeno' THEN v_base := 24; v_limit := 6;
  ELSIF v_size = 'grande' THEN v_base := 27; v_limit := 8;
  ...
  IF p_order_type = 'entrega' THEN v_delivery_fee := 7; END IF;
  v_total := v_base + v_shrimp_price + v_delivery_fee;

  INSERT INTO public.orders (
    customer_name, phone, order_type, address, number, complement, neighborhood, reference,
    size, base_price, pasta_type, sauce, ingredients, shrimp, shrimp_price, delivery_fee,
    saute_type, finishing, notes, total
  ) VALUES (
    trim(p_customer_name), coalesce(trim(p_phone),''), p_order_type,
    ...
    v_size, v_base, p_pasta_type, p_sauce, coalesce(p_ingredients, '{}'), coalesce(p_shrimp,false),
    v_shrimp_price, v_delivery_fee, p_saute_type, coalesce(p_finishing, '{}'),
